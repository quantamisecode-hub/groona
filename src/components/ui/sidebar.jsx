...
export function SidebarProvider({ children, defaultOpen = true, open: openProp, onOpenChange: setOpenProp }) {
  ...
  useEffect(() => {
    const checkMobile = () => setIsMobile(window.innerWidth < 1311);
  ...
  if (isMobile) {
    return (
      <>
        ...
        <div
          className={cn(
            "fixed inset-y-0 left-0 z-[100] w-[250px] bg-white shadow-lg transition-transform duration-300 ease-in-out",
  ...
  return (
    <aside
      className={cn(
        ...
        open ? "w-[230px]" : "w-[0px] overflow-hidden border-none",
        className
      )}
      {...props}
    >
