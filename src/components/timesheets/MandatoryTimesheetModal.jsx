...
export default function MandatoryTimesheetModal({ currentUser, effectiveTenantId }) {
    ...
    return (
        <div className={cn(
            "absolute inset-0 z-[90] flex items-center justify-center p-4 animate-in fade-in duration-300",
            hasAlarm ? "bg-slate-900/40 backdrop-blur-sm" : "pointer-events-none"
        )}>
            <div className="bg-white max-w-3xl w-full max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl border border-slate-200 overflow-hidden pointer-events-auto">
                <div className={`p-6 ${hasAlarm ? 'bg-red-50 border-b border-red-100' : 'bg-amber-50 border-b border-amber-100'}`}>
                    <div className="flex items-center gap-4">
                        ...
                        {/* Close button - only show if NOT hasAlarm */}
                        {!hasAlarm && (
                            <button
