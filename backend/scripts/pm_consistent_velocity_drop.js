...
async function checkAndAlertSingleSprintDrop() {
    try {
        ...
        console.log(`🔍 Checking ${projectsWithVelocity.length} projects for single sprint velocity drop...\n`);

        let lowVelocitySprints = [];

        // 2. Check each project for latest sprint velocity drop
        for (const projectId of projectsWithVelocity) {
            // Fetch the LATEST completed/active sprint
            const latestSprint = await SprintVelocity.findOne({
            ...
            // Check if accuracy is below 85%
            // Note: We include sprints with 0 committed points if they exist in SprintVelocity, 
            // but usually those have 0 accuracy anyway.
            if (latestSprint.accuracy < 85) {
                console.log(`⚠️  Project ${projectId} has low velocity (${latestSprint.accuracy.toFixed(1)}%):`);
        ...
        if (lowVelocitySprints.length === 0) {
            console.log('✅ No projects found with low velocity (<85%) in the latest sprint.\n');
            process.exit(0);
        }
        ...
        for (const velocityRecord of lowVelocitySprints) {
            try {
                ...
                const existingAlert = await Notification.findOne({
                    type: 'PM_CONSISTENT_VELOCITY_DROP', // Using specific type requested
                ...
                for (const recipient of recipients) {
                    ...
                    await Notification.create({
                        ...
                        type: 'PM_CONSISTENT_VELOCITY_DROP',
                        category: 'alert',
                        title: '⚠️ Sprint Velocity Drop Alert',
                        message: `⚠️ Sprint velocity has dropped below planned levels (${velocityRecord.accuracy.toFixed(1)}%). Review capacity and blockers.`,
                        entity_type: 'sprint',
                    ...
                    try {
                        await emailService.sendEmail({
                            to: recipient.email,
                            subject: `⚠️ Alert: Low Velocity Detected (${velocityRecord.accuracy.toFixed(1)}%)`,
        ...
        console.log('\n' + '='.repeat(60));
        console.log('🎯 VELOCITY DROP ALERT SUMMARY');
        console.log('='.repeat(60));
        console.log(`Total Projects with Low Velocity: ${lowVelocitySprints.length}`);
...
function getAlertHtml(velocityRecord) {
    const content = `
    <p style="${styles.text}">Sprint velocity has dropped below expected levels for your project <strong>${velocityRecord.project_name || 'Unknown Project'}</strong>.</p>
    
    <div style="${styles.infoBox}">
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Sprint:</span>
        <span style="${styles.value}"><strong>${velocityRecord.sprint_name}</strong></span>
      </div>
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Velocity Accuracy:</span>
        <span style="${styles.value}">
             <strong style="color: #ef4444;">${velocityRecord.accuracy.toFixed(1)}%</strong> (Threshold: 85%)
        </span>
      </div>
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Committed:</span>
        <span style="${styles.value}">${velocityRecord.committed_points} pts</span>
      </div>
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Completed:</span>
        <span style="${styles.value}">${velocityRecord.completed_points.toFixed(2)} pts</span>
      </div>
    </div>

    <p style="${styles.text}">Please review the sprint retrospective to identify blockers, check team capacity, or adjust scope as needed.</p>
    ...
    <div style="${styles.buttonGroup}">
        <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/project/${velocityRecord.project_id}/sprint/${velocityRecord.sprint_id}" 
           ...
           Open Velocity Comparison
        </a>
    </div>
    `;

    return getBaseTemplate(
        'Sprint Velocity Alert',
