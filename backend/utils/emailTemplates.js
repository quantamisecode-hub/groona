...
/**
 * Blocked By Assigned Template
 * Sent when a user is assigned to resolve a blocker on a task.
 */
function getBlockedByAssignedTemplate(data) {
  const { assigneeName, assigneeEmail, blockerTitle, taskTitle, projectName, assignedBy, dueDate, taskUrl } = data;

  const content = `
    <p style="${styles.text}">
      You have been assigned to resolve a <strong style="color: #d97706;">blocker</strong>
      for a task in the project <strong>${projectName || 'a project'}</strong>.
    </p>

    <div style="${styles.infoBox}; background-color: #fffbeb; border-color: #fcd34d;">
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Your Blocker:</span>
        <span style="${styles.value}"><strong style="color: #92400e;">${blockerTitle || 'N/A'}</strong></span>
      </div>
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Blocking Task:</span>
        <span style="${styles.value}">${taskTitle || 'N/A'}</span>
      </div>
      <div style="${styles.infoRow}">
        <span style="${styles.label}">Project:</span>
    ...
    <p style="${styles.text}">
      This task cannot be completed until this blocker is resolved. Please action this as soon as possible.
    </p>

    ${taskUrl ? `
      <div style="${styles.buttonGroup}">
        <a href="${taskUrl}" style="${styles.primaryBtn}; background-color: #d97706;">View Task</a>
      </div>
    ` : ''}
  `;
...
/**
 * Comment Mention / Task Comment Template
 * Sent when a user is mentioned in a comment or a PM is notified of a task comment.
 */
function getCommentMentionTemplate(data) {
  const { recipientName, recipientEmail, authorName, contextName, commentContent, entityType, entityId, projectUrl, taskUrl } = data;

  // Clean up and highlight the context string
  // 1. Bold the labels (Project, Sprint, Task)
  // 2. Bold and color the values
  // 3. Use a nicer separator
  const formattedContext = contextName
    .replace(/Project: \*\*(.*?)\*\*/g, '<strong>Project:</strong> <span style="color: #2563eb; font-weight: 600;">$1</span>')
    .replace(/Sprint: \*\*(.*?)\*\*/g, '<strong>Sprint:</strong> <span style="color: #2563eb; font-weight: 600;">$1</span>')
    .replace(/Task: \*\*(.*?)\*\*/g, '<strong>Task:</strong> <span style="color: #2563eb; font-weight: 600;">$1</span>')
    .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') // Generic fallback
    .replace(/ > /g, '<br/>');

  const content = `
    <p style="${styles.text}"><strong>${authorName}</strong> added a comment in:</p>
    <div style="margin-bottom: 24px; line-height: 1.8; background-color: #f1f5f9; padding: 12px 16px; border-radius: 6px; border-left: 4px solid #2563eb; font-size: 14px;">
      ${formattedContext}
    </div>
    
    <div style="${styles.infoBox}">
      <p style="margin: 0; font-style: italic; color: #475569; line-height: 1.6;">
        "${commentContent}"
      </p>
    </div>

    <p style="${styles.text}">You can view the full discussion and reply by clicking the button below.</p>

    <div style="${styles.buttonGroup}">
      <a href="${taskUrl || projectUrl || '#'}" style="${styles.primaryBtn}">View Comment</a>
    </div>
  `;

  // Subject line should be clean (no markdown tags)
  const cleanContext = contextName.replace(/\*\*/g, '');
  const subject = `New Comment in ${cleanContext}`;

  return {
    html: getBaseTemplate(
      'New Comment Notification',
      `Hello, ${recipientName || recipientEmail || 'Team Member'}`,
      content
    ),
